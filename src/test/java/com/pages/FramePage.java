...
public class FramePage extends PageObject {
	...
	public void switch_to_shop_frame() {
		getDriver().switchTo().frame(getDriver().findElement(By.cssSelector("iframe.s20iframe")));
	}
	...
	public void switch_to_quick_view_and_product_view_frame() {
		getDriver().switchTo().frame(getDriver().findElement(
				By.cssSelector("iframe[src*='storefront/product/']")));
	}

	// cart widget
	public void switch_to_cart_widget_frame() {
		getDriver().switchTo().frame(getDriver().findElement(
				By.cssSelector("iframe[src*='/storefront/cartwidgetPopup?cacheKiller']")));
	}

	// cart
	public void switch_to_cart_frame() {
		getDriver().switchTo().frame(getDriver().findElement(
				By.cssSelector("iframe[src*='/storefront/cart?cacheKiller']")));
	}
